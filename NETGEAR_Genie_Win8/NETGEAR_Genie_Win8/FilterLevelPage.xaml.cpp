//
// FilterLevelPage.xaml.cpp
// FilterLevelPage 类的实现
//
...
FilterLevelPage::FilterLevelPage()
{
	InitializeComponent();
}
...
void FilterLevelPage::LoadState(Object^ navigationParameter, IMap<String^, Object^>^ pageState)
{
...
void FilterLevelPage::SaveState(IMap<String^, Object^>^ pageState)
{
	(void) pageState;	// 未使用的参数
}

void FilterLevelPage::Next_Click(Object^ sender, ItemClickEventArgs^ e)
{
	(void) sender;	// 未使用的参数
	(void) e;		// 未使用的参数
	if (!SettingCompletePopup->IsOpen)
	{
		...
		ReturnToStatusPageButton->Visibility = Windows::UI::Xaml::Visibility::Visible;
	}
}

void FilterLevelPage::ReturnButton_Click(Object^ sender, ItemClickEventArgs^ e)
{
	(void) sender;	// 未使用的参数
	(void) e;		// 未使用的参数
	...
	if (SettingCompletePopup->IsOpen)
	{
		...
		ReturnToStatusPageButton->Visibility = Windows::UI::Xaml::Visibility::Collapsed;
		Frame->Navigate(TypeName(ParentalControlPage::typeid));
	}
}

void FilterLevelPage::RadioButton_Checked(Object^ sender, RoutedEventArgs^ e)
{
