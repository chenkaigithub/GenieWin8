    ...
    public class QRCodeEncoder
    {
		...
		public virtual bool[][] calQrcode(byte[] qrcodeData)
		{
			...
			try
			{
                ...
                String fileName = "QRCode.Resources.qrv" + Convert.ToString(qrcodeVersion) + "_" + Convert.ToString(ec) + ".dat";
                Stream x = GetResource(fileName);
			...
			try
			{
                ...
                string fileName = "QRCode.Resources.qrvfr" + Convert.ToString(qrcodeVersion) + ".dat";
                Stream x = GetResource(fileName);
                ...
                SystemUtils.ReadInput(x, frameData, 0, frameData.Length);
				//bis.Close();
                //bis.Dispose();
		...
		private static sbyte[] calculateRSECC(sbyte[] codewords, sbyte rsEccCodewords, sbyte[] rsBlockOrder, int maxDataCodewords, int maxCodewords)
		{
			...
			try
			{
			    ...
			    //StreamReader reader = new StreamReader(filename);
                String fileName = "QRCode.Resources.rsc" + rsEccCodewords.ToString() + ".dat";
				...
				for (int i = 0; i < 256; i++)
				{
					SystemUtils.ReadInput(x, rsCalTableArray[i], 0, rsCalTableArray[i].Length);
				}
				//bis.Close();
				//reader.Close();
        ...
        /// <returns></returns>
        public virtual QRCodeBitmapImage Encode(String content, Encoding encoding)  // Originally returned with WriteableBitmap type
        {
            try
            {
                ...
                //WriteableBitmap image = new WriteableBitmap((matrix.Length * qrCodeScale) + 1, (matrix.Length * qrCodeScale) + 1);
              

                QRCodeBitmapImage bitmapImage = new QRCodeBitmapImage(null, (matrix.Length * qrCodeScale) + 1, (matrix.Length * qrCodeScale) + 1); 

                //image.FillRectangle(0, 0, image.PixelWidth, image.PixelHeight, qrCodeBackgroundColor);


                bitmapImage.FillRectangle(0, 0, bitmapImage.Width, bitmapImage.Height, qrCodeBackgroundColor);
                ...
                for (int i = 0; i < matrix.Length; i++)
                {
                    for (int j = 0; j < matrix.Length; j++)
                    {
                        if (matrix[j][i])
                        {
                            ...
                            int x2 = j * qrCodeScale + qrCodeScale;
                            int y2 = i * qrCodeScale + qrCodeScale;

                            //image.FillRectangle(j * qrCodeScale, i * qrCodeScale, x2, y2, qrCodeForegroundColor);

                            bitmapImage.FillRectangle(j * qrCodeScale, i * qrCodeScale, x2, y2, qrCodeForegroundColor);
            ...
            {
                //MessageDialog md = new MessageDialog("Exception occured Fill Rectangle " + ex.Message);
                //md.ShowAsync();
                throw new Exception("Encode",ex);
                return null;
            }
        }
        ...
        /// <param name="encoding"></param>
        /// <returns></returns>
        public virtual QRCodeBitmapImage Encode(String content) // Originally returned with WriteableBitmap type
        {
        ...
        public static Stream GetResource(string fileName)
        {
            ...
            //System.Reflection.Assembly a = typeof(QRCodeEncoder).GetTypeInfo().Assembly;
            //typeof(QRCodeEncoder).AssemblyQualifiedName



            return typeof(QRCodeEncoder).GetTypeInfo().Assembly.GetManifestResourceStream(fileName);
