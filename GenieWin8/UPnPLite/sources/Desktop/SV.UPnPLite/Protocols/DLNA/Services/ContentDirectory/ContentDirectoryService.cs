...
namespace SV.UPnPLite.Protocols.DLNA.Services.ContentDirectory
{
    ...
    {
        public static string strDIDLLite;
        #region Fields
        ...
        private static IEnumerable<MediaObject> ParseMediaObjects(string mediaObjectsXml)
        {
            ...
            using (var reader = new StringReader(mediaObjectsXml))
            {
                using (var xmlReader = XmlReader.Create(reader, xmlReaderSettings))
                {
                    ...
                    if (xmlReader.Name == "DIDL-Lite")
                    {
                        if (mediaObjectsXml.Contains("\n"))
                        {
                            var index = mediaObjectsXml.IndexOf("\n");
                            strDIDLLite = mediaObjectsXml.Substring(0, index);
                        }
                        else
                        {
                            var index = mediaObjectsXml.IndexOf(">");
                            strDIDLLite = mediaObjectsXml.Substring(0, index + 1);
                        }
                    }
                    xmlReader.Read();
