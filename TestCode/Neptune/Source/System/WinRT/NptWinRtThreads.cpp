...
/*----------------------------------------------------------------------
|   NPT_WinRtThread::GetThreadPriority
+---------------------------------------------------------------------*/
NPT_Result
	NPT_WinRtThread::GetThreadPriority(HANDLE thread, int priority)
{
#if 0
	int WinRt_priority = ::GetThreadPriority(thread);
	if (WinRt_priority == THREAD_PRIORITY_IDLE) {
		priority = NPT_THREAD_PRIORITY_LOWEST - 1;
	} else if (WinRt_priority == THREAD_PRIORITY_LOWEST) {
		priority = NPT_THREAD_PRIORITY_BELOW_NORMAL - 1;
	} else if (WinRt_priority == THREAD_PRIORITY_BELOW_NORMAL) {
		priority = NPT_THREAD_PRIORITY_NORMAL - 1;
	} else if (WinRt_priority == THREAD_PRIORITY_NORMAL) {
		priority = NPT_THREAD_PRIORITY_ABOVE_NORMAL - 1;
	} else if (WinRt_priority == THREAD_PRIORITY_ABOVE_NORMAL) {
		priority = NPT_THREAD_PRIORITY_HIGHEST - 1;
	} else if (WinRt_priority == THREAD_PRIORITY_HIGHEST) {
		priority = NPT_THREAD_PRIORITY_TIME_CRITICAL - 1;
	} else {
		priority = NPT_THREAD_PRIORITY_TIME_CRITICAL;
	}
	return priority;
#endif
	return NPT_SUCCESS;
...
{
	if (m_ThreadHandle == 0) return NPT_ERROR_INVALID_STATE;
	return NPT_WinRtThread::GetThreadPriority(m_ThreadHandle, priority);
}
